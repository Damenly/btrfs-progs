...
next:
	...
	case BTRFS_TREE_BLOCK_REF_KEY:
		...
		owner = level;
		tmp_err = check_tree_block_backref(fs_info, offset, key.objectid,
					       level);
		break;
	case BTRFS_SHARED_BLOCK_REF_KEY:
		parent = offset;
		tmp_err = check_shared_block_backref(fs_info, offset, key.objectid,
						 level);
		break;
	case BTRFS_EXTENT_DATA_REF_KEY:
		dref = (struct btrfs_extent_data_ref *)(&iref->offset);
		...
		tmp_err = check_extent_data_backref(fs_info, root_objectid, owner,
					owner_offset, key.objectid, key.offset,
					btrfs_extent_data_ref_count(eb, dref));
		break;
	case BTRFS_SHARED_DATA_REF_KEY:
		parent = offset;
		tmp_err = check_shared_data_backref(fs_info, offset, key.objectid);
		break;
	default:
